...
function SearchConsoleIcon() {
  return (
    <svg viewBox="0 0 24 24" className="size-10 shrink-0" aria-hidden>
      <circle cx="10.5" cy="10.5" r="6.5" fill="none" stroke="#4285F4" strokeWidth="2.2" />
      <line x1="15.2" y1="15.2" x2="21" y2="21" stroke="#34A853" strokeWidth="2.4" strokeLinecap="round" />
    </svg>
  );
}

type SearchConsoleConnectCardProps = {
  domain: string;
  onConnect: () => void;
...
export function SearchConsoleConnectCard({
  ...
  onConnect,
  isConnecting = false,
}: SearchConsoleConnectCardProps) {
  const t = useTranslations("searchConsole");

  return (
    <div className="flex w-full max-w-lg flex-col items-center gap-6 text-center">
      <div className="flex size-20 items-center justify-center rounded-full bg-[#E8F0FE]">
        <SearchConsoleIcon />
      </div>
